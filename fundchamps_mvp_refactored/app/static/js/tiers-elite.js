(() => {
  try {
    const d = document,
      cards = [...d.querySelectorAll("#tiers .s-tier-card")];
    ...
    cards.forEach((card) => {
      ...
      if (!card.querySelector(".s-tier-badge")) {
        ...
        b.className = "s-tier-badge";
        b.textContent = emoji[m] + " " + m.charAt(0).toUpperCase() + m.slice(1);
        card.appendChild(b);
      }
      card.querySelectorAll("a,button").forEach((btn) => {
        if (/sponsor/i.test(btn.textContent)) {
          ...
          btn.addEventListener("click", () => {
            try {
              localStorage.setItem("fc_last_tier", m);
            } catch (e) {}
            showSticky(m);
          });
        }
    ...
    function showSticky(name) {
      ...
      if (!bar) {
        ...
        bar.id = "tiers-sticky";
        bar.innerHTML = '<div class="wrap">Selected: <b></b></div>';
    ...
    const want = (
      ...
      localStorage.getItem("fc_last_tier") ||
      ""
    ).toLowerCase();
    if (want) {
      const hit = cards.find((c) => c.dataset.tier === want);
      if (hit) hit.classList.add("in-view");
    }
