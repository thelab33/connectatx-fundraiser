...
(() => {
  ...
  const csrfToken = () =>
    document.querySelector('meta[name="csrf-token"]')?.content || getCookie('csrf_token') || '';

  // Broadcast channel (optional)
  let bus = null;
  try { bus = new BroadcastChannel('fc_ui'); } catch {}
  ...
  const fc = {
    ...
    bus,
    csrf: csrfToken,
    on: (type, fn, opts) => { window.addEventListener(type, fn, opts); return () => window.removeEventListener(type, fn, opts); },
