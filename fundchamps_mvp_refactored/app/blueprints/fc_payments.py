...
def _emit(channel, payload):
    try:
        ...
        current_app.socketio.emit(
            channel,
            payload,
            namespace="/donations" if channel == "donation" else "/sponsors",
        )
    except Exception:
        pass
...
def _tier_for(amount):
    ...
    return (
        ...
        if a >= 2500
            ...
            "Gold"
            if a >= 1000
            else "Silver" if a >= 500 else "Bronze" if a >= 250 else "Community"
        )
    )
...
def stripe_webhook():
    ...
    if et in ("payment_intent.succeeded", "charge.succeeded"):
        ...
        who = (
            meta.get("name")
            or (data.get("billing_details", {}) or {}).get("name")
            or "Supporter"
        )
        pay = {
            "name": who,
...
def paypal_capture(order_id):
    ...
    if resp.result.status in ("COMPLETED", "APPROVED"):
        ...
        name = (
            (resp.result.payer.name.given_name or "Supporter")
            if resp.result.payer and resp.result.payer.name
            else "Supporter"
        )
        pay = {
            "name": name,
