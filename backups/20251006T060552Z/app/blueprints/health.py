...
BUILD_VERSION = (os.getenv("BUILD_VERSION") or os.getenv("RELEASE") or os.getenv("VERSION") or "dev")
...
def _redis_check() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "degraded" if not STRICT_HEALTH else "fail", "ok": False, "url": REDIS_URL, "error": str(e)}
...
def _stripe_check() -> Dict[str, Any]:
    ...
    if not stripe:
        return {"status": "fail" if STRICT_HEALTH else "degraded", "ok": False, "reason": "stripe-lib-missing"}
    status = {"status": "ok", "ok": True, "mode": "live" if ("_live_" in key or key.startswith("sk_live_")) else "test"}
    if DEEP_CHECKS:
        try:
            stripe.api_key = key  # type: ignore
            acct = stripe.Account.retrieve()  # type: ignore
            status["account"] = {"id": getattr(acct, "id", None), "charges_enabled": getattr(acct, "charges_enabled", None)}
        except Exception as e:
            status = {"status": "degraded" if not STRICT_HEALTH else "fail", "ok": False, "error": str(e)}
    return status
...
def _summary_payload() -> Dict[str, Any]:
    ...
    return {
        ...
        "git": GIT_SHA,
        "hostname": HOSTNAME,
        "started_at": datetime.fromtimestamp(APP_STARTED_AT, tz=timezone.utc).isoformat(timespec="seconds"),
...
@bp.get("/live")
def live():
    return jsonify({"status": "ok", "now": _now_iso(), "uptime_s": int(time.time() - APP_STARTED_AT)})
