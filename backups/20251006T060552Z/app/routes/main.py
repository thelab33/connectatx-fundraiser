...
"""
Main web blueprint: homepage, donor/sponsor flows, static pages, sheets, and stats API.
...
# ── Sheet helpers & routes ─────────────────────────────────────────
def _current_team() -> Dict[str, Any]:
    team = getattr(g, "team", None)
    return team or TEAM_CONFIG or {}


def _get_tiers_for_team(team) -> List[Any]:
    try:
        from app.models import Tier  # type: ignore
        return list(getattr(Tier, "for_team")(team))  # type: ignore[attr-defined]
    except Exception:
        return []


def _render_embed(template_name: str, **ctx):
    """Return partial markup (no base.html) for sheet fetches."""
    try:
        ...
        resp = current_app.response_class(
            ...
            200,
            {
                ...
                "Vary": "X-Requested-With, X-Partial",
            },
        )
        return resp
    except Exception:
        current_app.logger.exception("Embed render failed: %s", template_name)
        return make_response("Failed to load content.", 500)


@bp.get("/tiers")
def embed_tiers():
    mode = request.args.get("mode")
    template = "embed/tiers_inline.html" if mode == "inline" else "embed/tiers_sheet.html"
    return render_template(template, tiers=TIERS_DATA)
...
def embed_impact():
    team = _current_team()
    impact = _generate_impact_stats(TEAM_CONFIG)
    return _render_embed("embed/impact_sheet.html" if request.headers.get("X-Partial") else "impact.html", team=team, impact=impact)


@bp.get("/embed/about")
def embed_about():
    team = _current_team()
    return _render_embed("embed/about_sheet.html", team=team)
