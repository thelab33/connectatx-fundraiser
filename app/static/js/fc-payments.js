...
(() => {
  if (window.__fcPayments) return; window.__fcPayments = true;
  ...
  async function ensureStripe(){
    ...
    } catch(e){ console.error("fc-payments catch:", e); }
    return stripe;
  }
  ...
  window.addEventListener("fc:donate:open", (ev)=>{
    ...
    if (input && has(amt)) { input.value = String(amt); input.dispatchEvent(new Event("input",{bubbles:true})); }
  ...
  document.addEventListener("click", (e)=>{
    ...
    const amt = Math.max(10, Math.round(base * 0.10)); // ensure >= $10
  ...
  async function handleStripeSubmit(e){
    ...
    e.preventDefault();
    const amount = parseFloat((form.querySelector("#donation-amount")||{}).value||"0")||0;
    ...
    const res = await fetch("/api/payments/stripe/intent", {
      ...
      body: JSON.stringify({ amount, name, email, frequency: (form.querySelector('input[name="frequency"]:checked')||{}).value || "once" })
    });
    const { client_secret } = await res.json();
    const { error } = await stripe.confirmPayment({ elements, clientSecret: client_secret, confirmParams: { return_url: window.location.href }});
    if (error) {
      console.error(error.message || error);
      alert("Payment error: " + (error.message||"Please try again."));
    }
  }
  document.getElementById("sponsor-donation-form")?.addEventListener("submit", handleStripeSubmit);
  ...
  if (window.paypal && document.getElementById("paypal-buttons")) {
    ...
    paypal.Buttons({
      createOrder: async ()=> {
        const r = await fetch("/api/payments/paypal/order", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ amount: getAmt() }) });
        return (await r.json()).id;
      },
      onApprove: async (data)=> {
        await fetch(`/api/payments/paypal/capture/${data.orderID}`, { method:"POST" });
  ...
  const postJSON = (u,b)=> fetch(u,{method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify(b)}).catch(()=>{});
  const hub = document.getElementById("sponsors-hub");
  if (hub && "IntersectionObserver" in window) {
    const fire = once(()=> postJSON("/api/metrics/impression", { key:"sponsors_hub" }));
    new IntersectionObserver((entries)=> entries.forEach(e=> { if (e.isIntersecting) fire(); }), { threshold: 0.2 }).observe(hub);
  }
  document.addEventListener("click",(e)=>{
    const a = e.target.closest('a[rel~="sponsored"], a[data-sponsor]');
    if (a) postJSON("/api/metrics/click", { key: a.getAttribute("href")||"sponsor", name: a.getAttribute("data-sponsor-name")||"" });
  });
  ...
  if (typeof window.io === "function") {
    try {
      ...
      s1.on("donation", (d)=> {
        const a = +d.amount||0;
        if (window.confetti) window.confetti({ particleCount: Math.min(200, 40 + Math.round(a/5)), spread: 70, origin: { y: 0.7 } });
        if (window.dataLayer) window.dataLayer.push({ event:"donation", amount:a, tier:d.tier||"Community" });
      });
      const s2 = io("/sponsors", { transports:["websocket","polling"] });
      s2.on("sponsor", (d)=> {
        const big = (+d.amount||0) >= 1000;
        if (window.confetti) window.confetti({ particleCount: big?220:120, spread: big?90:60, origin:{y:0.7} });
        if (window.dataLayer) window.dataLayer.push({ event:"sponsor_spotlight", amount:+d.amount||0, tier:d.tier||"Community" });
      });
    } catch(e){ console.error("fc-payments catch:", e); }
  }
})();
