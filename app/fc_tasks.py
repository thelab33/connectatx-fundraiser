...
celery = Celery("fc", broker=REDIS_URL, backend=REDIS_URL, timezone=os.getenv("TIMEZONE","America/Chicago"))
...
def send_weekly_roi():
    ...
    lines = [f"FundChamps Weekly ROI ({wk})",
             f"Impressions: {h.get('impressions','0')}",
             f"Clicks:      {h.get('clicks','0')}",
             f"Donations:   {h.get('donations_count','0')} • $ {h.get('donations_total','0')}"]
    lines.append("\nRecent donations:")
    for d in recent:
        lines.append(f" - {d.get('name','Supporter')}: $ {d.get('amount',0)} at {d.get('at','')}")
    body = "\n".join(lines)
    to = os.getenv("ROI_REPORT_TO")
    api = os.getenv("SENDGRID_API_KEY")
    if api and to:
        msg = Mail(from_email="no-reply@fundchamps.app", to_emails=to, subject="FundChamps • Weekly Sponsor ROI", plain_text_content=body)
...
celery.conf.beat_schedule = {
    "weekly-roi": { "task": "fc_tasks.send_weekly_roi", "schedule": crontab(minute=0, hour=8, day_of_week="mon") }
}
