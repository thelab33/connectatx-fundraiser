...
_NUM_RE = re.compile(r"[-+]?\$?\s*([0-9]{1,3}(?:,[0-9]{3})*|[0-9]+)(?:\.[0-9]+)?\s*([KkMm])?$")
...
def pct(n: Any, d: Any) -> float:
    ...
    n = parse_money(n); d = parse_money(d)
    if d <= 0:
        return 0.0
    return (n / d) * 100.0

def _calc_next_milestone_gap(total: Any, allocated: Any, milestones: Iterable[Dict[str, Any]]) -> Tuple[float, str]:
    """
